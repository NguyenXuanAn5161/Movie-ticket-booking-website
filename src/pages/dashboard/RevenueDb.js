...
export const StatisticByMovie = (listData, dateRanger, movie) => {
  ...
  if (listData.length > 0) {
    ...
    // Merge các ô từ tiêu đề đến ô cuối cùng của tiêu đề
    const titleCount =
      listData.length > 0 ? Object.keys(listData[0]).length - 2 : 0;
    const mergeEndCell = worksheet.getCell(getColumnLetter(titleCount) + "1");
    worksheet.mergeCells("A1:" + mergeEndCell.address);
    titleCell.alignment = { horizontal: "center", vertical: "middle" };
    ...
    nameMoviecell.font = { italic: true, size: 10, name: "Times New Roman" }; // Thiết lập font chữ cho tên rạp
    ...
    listData.forEach((data) => {
      ...
      worksheet.getColumn(4).numFmt = "#,##0.00"; // Định dạng số cho cột "Tổng doanh thu"
    });
    ...
    // Ghi các giá trị tổng vào hàng tổng cộng
    totalRow.getCell(2).value = totalInvoice;
    totalRow.getCell(3).value = totalTicket;
