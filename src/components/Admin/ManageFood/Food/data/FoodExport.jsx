...
const FoodExport = (props) => {
  const { setOpenModalExport, openModalExport, listFood } = props;
  ...
  const handleExportData = () => {
    ...
    if (listFood.length > 0) {
      const worksheet = XLSX.utils.json_to_sheet(listFood);
      const workbook = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(workbook, worksheet, "Sheet1");
      XLSX.writeFile(workbook, `ExportBook.${selectedFormat}`);
    }
  ...
  return (
    <>
      <Modal
        title="Xuất tệp dữ liệu đồ ăn dưới dạng"
        width={"30vw"}
